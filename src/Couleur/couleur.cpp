...
uint8_t muterCanal(uint8_t v, double amplitude, std::mt19937& graine)
{
    int val=v;
    val += util::alea(-amplitude, amplitude, graine);
...
void Couleur::muter(double amplitude, std::mt19937& graine)
{
    m_rouge = muterCanal(m_rouge, amplitude, graine);
    m_vert = muterCanal(m_vert, amplitude, graine);
    m_bleu = muterCanal(m_bleu, amplitude, graine);
}
